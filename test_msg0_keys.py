...
# Page 1 layout: [16-byte salt] [encrypted] [reserve]
salt = page1[:16]
encrypted = page1[16:PAGE_SIZE - RESERVE]
reserve = page1[PAGE_SIZE - RESERVE:]
iv = reserve[:IV_SIZE]

print(f"MSG0.db salt: {salt.hex()}")
print(f"MSG0.db IV: {iv.hex()}")
...
for i, hex_key in enumerate(keys_hex):
    ...
    if dec[:2] == b'\x10\x00':
        print(f"MATCH (page_size check)! Key {i}: {hex_key}")
        print(f"  Decrypted start: {dec[:32].hex()}")

# Also try: maybe the zero salt means there's no salt and the whole page is encrypted
# i.e., encrypted = page1[:PAGE_SIZE - RESERVE], reserve = page1[PAGE_SIZE - RESERVE:]
print("\nAlternative: no salt prefix...")
encrypted2 = page1[:PAGE_SIZE - RESERVE]
reserve2 = page1[PAGE_SIZE - RESERVE:]
iv2 = reserve2[:IV_SIZE]
print(f"  IV: {iv2.hex()}")

for i, hex_key in enumerate(keys_hex):
    key = bytes.fromhex(hex_key)
    cipher = AES.new(key, AES.MODE_CBC, iv2)
    dec = cipher.decrypt(encrypted2[:48])
    # If no salt, the start should be the full SQLite header
    if dec[:16] == SQLITE_HEADER or dec[:4] == b"SQLi":
        print(f"  MATCH (SQLite header)! Key {i}: {hex_key}")
        print(f"  Decrypted start: {dec[:32]}")
    # Or it might start with 0x10 0x00 (after implicit 16-byte header area)
    if dec[:2] == b'\x10\x00':
        print(f"  MATCH (page_size)! Key {i}: {hex_key}")
