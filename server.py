...
@app.post("/api/transcribe/enqueue")
...
@app.post("/api/transcribe/enqueue-upload")
...
@app.delete("/api/pdf/jobs/{job_id}")
...
# ---------------------------------------------------------------------------
# Excel string search — find a string across selected sheets of Excel files
# ---------------------------------------------------------------------------
...
@app.post("/api/excel-search/structure")
...
@app.post("/api/excel-search/export")
...
@app.post("/api/teams-transcript/enqueue")
...
@app.get("/api/teams-transcript/jobs")
...
@app.get("/api/teams-transcript/status/{job_id}")
...
@app.delete("/api/teams-transcript/jobs/{job_id}")
...
# ---------------------------------------------------------------------------
_TEAMS_CHAT_SELECTOR_FILE = os.path.join(os.path.dirname(__file__), "teams_chat_selectors.json")


@app.get("/api/teams-chat/selectors")
...
@app.put("/api/teams-chat/selectors")
...
@app.post("/api/teams-chat/diagnose/stream")
...
@app.post("/api/copilot-chat/export/stream")
async def copilot_chat_export_stream(req: CopilotChatExportRequest, user: User = Depends(require_user)):
    ...
    def run_worker():
        worker = os.path.join(os.path.dirname(__file__), "copilot_chat_worker.py")
        fmt = req.format if req.format in ("html", "txt") else "html"
        suffix = ".html" if fmt == "html" else ".txt"
        tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False)
        tmp.close()
...
@app.get("/api/copilot-chat/download/{job_id}")
